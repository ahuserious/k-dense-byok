 ...
 *
 * FALSE UNTIL TWO THINGS LAND, both quoted in `INTEGRATION.md` at the clone root:
 *
 *   1. `registerSubscriptionUsageRoutes(app)` in `server/src/index.ts` — without
 *      it the route 404s (harmless: the session fallback still shows real data).
 *   2. A `/subscription-usage` line in `e2e/fixtures.ts` — this one is NOT
 *      harmless. `installApiMocks` records every unmocked path in
 *      `apiState.unexpectedRequests`, and the auto `runtimeErrors` fixture
 *      asserts that array is empty, so an unmocked call from the app-shell
 *      header would fail EVERY Playwright spec in the suite, not just this
 *      lane's. Neither file is in lane F8's writable set.
 *
 * When both are applied this becomes `true` and nothing else changes — the
 * upgrade path is exercised today by `subscription-bar.test.tsx`, which passes
 * the override explicitly.
 */
export const PROJECT_ROLLUP_ROUTE_ENABLED = false;
