 ...
 * terms.
 *
 * IMPORTANT — loop EXECUTION is not wired here. The reference tree drove loops
 * with an in-process orchestrator built on pre-0.42 agent internals, which the
 * E1 port plan deliberately excludes. The lifecycle routes below operate on the
 * PERSISTED loop doc only (create / pause / resume / stop change status + the
 * iteration cap via agent/goal-loop.ts); a created loop is recorded and
 * observable but no agent runs are dispatched for it. The read routes (runs /
 * loops listing) are fully functional today and surface every recorded run —
 * e.g. the /pipelines proxy's workflow rows — which is the backfill goal.
 */
...
export async function registerConsoleRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post<{ Params: { id: string }; Body: { extraIterations?: unknown } }>(
    ...
    async (req, reply) => {
      ...
      // iteration cap by extraIterations (and, once an engine exists, re-kicks it).
...
/**
 * Create a goal loop. startLoop (agent/goal-loop.ts) mints the persisted loop
 * doc — status "pending" until a loop engine lands (see the header). It
 * captures projectId explicitly so nothing here depends on the request's
 * AsyncLocalStorage scope after this handler returns.
 */
