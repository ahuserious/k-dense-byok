...
export const MAX_LEGACY_PIPELINE_WORKFLOW_BYTES = 512 * 1024;
...
export type LegacyPipelineImportIssueSeverity = "warning" | "blocker";

export interface LegacyPipelineImportIssue {
  severity: LegacyPipelineImportIssueSeverity;
  code: string;
  path: string;
  message: string;
}

export interface LegacyPipelineImportPreview {
  sourceFormat: "pipeline-workflow-yaml/v1";
  sourceSha256: string;
  graph: WorkflowGraphDocument | null;
  warnings: LegacyPipelineImportIssue[];
  blockers: LegacyPipelineImportIssue[];
  legacyRuns: {
    mode: "archive-only";
    resumable: false;
    reason: string;
...
export type LegacyPipelineImportErrorCode =
  | "INVALID_IMPORT_REQUEST"
  | "INVALID_LEGACY_YAML"
  | "LEGACY_SOURCE_TOO_LARGE";

export class LegacyPipelineImportError extends Error {
  constructor(
    readonly code: LegacyPipelineImportErrorCode,
    message: string,
  ) {
    super(message);
    this.name = "LegacyPipelineImportError";
  }
}
...
export interface PreviewLegacyPipelineWorkflowOptions {
  source: string;
  workflowId: string;
...
function legacyRunBoundary(): LegacyPipelineImportPreview["legacyRuns"] {
  return {
    mode: "archive-only",
    resumable: false,
    reason:
...
export function previewLegacyPipelineWorkflow(
  options: PreviewLegacyPipelineWorkflowOptions,
): LegacyPipelineImportPreview {
  ...
  if (Buffer.byteLength(options.source, "utf8") > MAX_LEGACY_PIPELINE_WORKFLOW_BYTES) {
    throw new LegacyPipelineImportError(
      "LEGACY_SOURCE_TOO_LARGE",
      `Legacy workflow YAML exceeds ${MAX_LEGACY_PIPELINE_WORKFLOW_BYTES} bytes.`,
    );
  }
  ...
  const warnings: LegacyPipelineImportIssue[] = [];
  const blockers: LegacyPipelineImportIssue[] = [];
  const baseResult = {
    sourceFormat: "pipeline-workflow-yaml/v1" as const,
    sourceSha256,
    warnings,
    blockers,
  ...
  if (parsed.provider !== "pi") {
    ...
    warnings.push(issue(
      ...
      "/provider",
      "The legacy Pi runtime marker maps to Pi (Kady); the Pipeline engine sidecar is not retained.",
    ));
  }
