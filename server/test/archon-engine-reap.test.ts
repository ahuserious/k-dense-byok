...
/**
 * start.mjs spawns the vendored workflow engine (server/vendor/archon-engine)
...
const VENDOR_DIR = path.join(process.cwd(), "vendor", "archon-engine");
...
describe("vendored workflow engine owned-tree reaping", () => {
  it.skipIf(!runnable)(
    ...
    async () => {
      ...
      // backend/frontend ports; ARCHON_HOME keeps engine state out of ~/.archon.
      const enginePort = 3400 + (process.pid % 500);
      const engineHome = fs.mkdtempSync(path.join(os.tmpdir(), "kady-archon-reap-"));
