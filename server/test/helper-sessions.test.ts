...
describe("dedicated helper Pi sessions", () => {
  ...
  it("isolates Workflow Rescue per failed run and rejects bypass payloads and non-failed state", async () => {
    ...
    expect(first.statusCode).toBe(200);
    expect(first.json().readOnlyTools).toEqual(["workflow_rescue_read"]);
    ...
    expect(rescueSession?.getActiveToolNames()).toEqual(["workflow_rescue_read"]);
    ...
    expect(reopenedRescueSession?.getActiveToolNames()).toEqual(["workflow_rescue_read"]);
