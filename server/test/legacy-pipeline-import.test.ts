...
} from "../src/workflows/legacy-pipeline-import.ts";

function preview(source: string) {
  return previewLegacyPipelineWorkflow({
    source,
...
describe("legacy Pipeline engine workflow preview", () => {
...
nodes:
  - id: start
    prompt: Run once.
    model: ollama/local
`)).toThrowError(LegacyPipelineImportError);
