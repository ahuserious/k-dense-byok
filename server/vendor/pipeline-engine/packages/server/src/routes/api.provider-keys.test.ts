...
// --- Identity resolution (X-Pipeline-User → user) ---
...
const ALICE = { 'X-Pipeline-User': 'alice' };
    ...
    mockStartOAuth.mockRejectedValueOnce(
      new OAuthCallbackPortBusyError(
        "A previous 'anthropic' login attempt is still holding the OAuth callback port. " +
          'Wait a few seconds and retry; if it persists, restart the Pipeline Engine server.'
      )
    );
    const res = await makeApp().request('/api/auth/providers/claude/oauth/start', {
      method: 'POST',
...
// EVERY Pipeline Engine-registered /api/auth/* route must be in isArchonOwnedAuthPath or it
// 404s live (this is exactly what bit GET /api/auth/providers). registerApiRoutes
// registers only Pipeline Engine routes (Better Auth mounts separately in index.ts), so
// all /api/auth/* paths here are Pipeline Engine's and must be exempted.
describe('Better Auth catch-all exemption is exhaustive', () => {
  test('every Pipeline Engine-registered /api/auth/* route is exempted by isArchonOwnedAuthPath', () => {
    const app = makeApp();
    const authPaths = [
