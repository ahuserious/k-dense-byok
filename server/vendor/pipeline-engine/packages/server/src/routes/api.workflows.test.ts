...
mock.module('@archon/workflows/defaults', () => ({
  BUNDLED_WORKFLOWS: {
    'pipeline-assist': 'name: pipeline-assist\ndescription: Pipeline Engine Assist\nnodes: []',
  },
  BUNDLED_COMMANDS: {
    'pipeline-assist': '# pipeline-assist command',
  },
...
describe('GET /api/workflows/:name', () => {
  ...
  test('returns bundled workflow with source:bundled', async () => {
    ...
    // No cwd → no readFile attempt → checks BUNDLED_WORKFLOWS → pipeline-assist found
    mockListCodebases.mockImplementationOnce(async () => []);

    const response = await app.request('/api/workflows/pipeline-assist');
    expect(response.status).toBe(200);
    const body = (await response.json()) as { source: string; filename: string; workflow: unknown };
    expect(body.source).toBe('bundled');
    expect(body.filename).toBe('pipeline-assist.yaml');
...
describe('GET /api/workflows/:name - cwd validation', () => {
  test('returns 400 when cwd is not a registered codebase path', async () => {
    ...
    const response = await app.request('/api/workflows/pipeline-assist?cwd=/etc/secrets');
    expect(response.status).toBe(400);
    const body = (await response.json()) as { error: string };
...
describe('PUT /api/workflows/:name', () => {
  ...
  test('falls back to getArchonHome() when no cwd and no codebases registered', async () => {
    const testArchonHome = join(tmpdir(), `pipeline-home-test-${Date.now()}`);
  ...
  test('saves valid workflow to ARCHON_HOME workflows when source=global', async () => {
    const testArchonHome = join(tmpdir(), `pipeline-home-put-global-${Date.now()}`);
    process.env.ARCHON_HOME = testArchonHome;
...
describe('DELETE /api/workflows/:name', () => {
  test('returns 400 for a mocked bundled default name', async () => {
    const app = createTestApp();
    registerApiRoutes(app, {} as WebAdapter, {} as ConversationLockManager);

    // The module mock supplies this bundled entry to exercise deletion protection.
    const response = await app.request('/api/workflows/pipeline-assist', { method: 'DELETE' });
    expect(response.status).toBe(400);
    const body = (await response.json()) as { error: string };
    expect(body.error).toContain('pipeline-assist');
  });
  ...
  test('removes home-scoped workflow file when source=global', async () => {
    const testArchonHome = join(tmpdir(), `pipeline-home-del-global-${Date.now()}`);
...
describe('GET /api/commands', () => {
  ...
  test('includes mocked bundled commands with source:bundled', async () => {
    const app = createTestApp();
    registerApiRoutes(app, {} as WebAdapter, {} as ConversationLockManager);
    ...
    expect(response.status).toBe(200);
    const body = (await response.json()) as { commands: Array<{ name: string; source: string }> };
    // The module mock supplies this command to exercise source labelling.
    const bundledAssist = body.commands.find(c => c.name === 'pipeline-assist');
    expect(bundledAssist).toBeDefined();
    expect(bundledAssist?.source).toBe('bundled');
  });
  ...
  test.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const projectDir = join(
        tmpdir(),
        `pipeline-api-commands-${Date.now()}-${Math.random().toString(36).slice(2)}`
      );
      const sourceDir = join(
        tmpdir(),
        `pipeline-api-commands-source-${Date.now()}-${Math.random().toString(36).slice(2)}`
      );
