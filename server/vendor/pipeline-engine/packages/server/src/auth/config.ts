...
/**
 * Paths under `/api/auth/*` that Pipeline Engine owns and the Better Auth catch-all must
 ...
 * The mount falls through (`next()`) for these so Pipeline Engine's own route handlers
 * (registered later in `registerApiRoutes`) run instead.
 *
 * Keep this in lockstep with the Pipeline Engine-owned `/api/auth/*` routes:
 ...
 * single source of truth for "this prefix is Pipeline Engine's, not Better Auth's".
 */
