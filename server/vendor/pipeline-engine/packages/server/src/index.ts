...
// Load pipeline-owned env from ~/.archon/.env (user scope) and <cwd>/.archon/.env
 ...
 * Discord snowflake) to an Pipeline Engine user UUID via auto-create-on-first-sight.
 *
...
export async function startServer(opts: ServerOptions = {}): Promise<void> {
  ...
  // buttons in the Web UI dashboard, or `pipeline-engine workflow abandon <run-id>`.
  // (`pipeline-engine workflow cleanup` is a separate command that deletes OLD terminal
  // rows for disk hygiene — it does not handle stuck `running` rows.)
  // See #1216.

  // Log Pipeline Engine paths configuration
  logArchonPaths();
  ...
  // Stream workflow runs started in ANY process (incl. the `pipeline-engine` CLI / `--detach`)
  ...
  if (!opts.skipPlatformAdapters) {
    ...
    // set in /etc/pipeline/.env. (PRD: "fail-fast on misconfig".)
    ...
    if (ghAuthMode.kind === 'app') {
      ...
      githubAppAuthProvider = createGitHubAppAuthProvider({
        appId,
        privateKey,
        slug: process.env.GITHUB_APP_SLUG ?? 'pipeline',
    ...
    if (process.env.DISCORD_BOT_TOKEN) {
      ...
      discordAdapter.onMessage(async ({ message, platformUserId, displayName }) => {
        ...
        // Resolve Discord author → Pipeline Engine user UUID.
      ...
      // `pipeline-engine serve` for the web UI shouldn't lose it because of an
    ...
    if (process.env.SLACK_BOT_TOKEN && process.env.SLACK_APP_TOKEN) {
      ...
      slackAdapter.onMessage(async event => {
        ...
        // Resolve Slack user → Pipeline Engine user UUID. displayName comes from
  ...
  // that prefix. Pipeline Engine ALSO owns a few /api/auth/* routes (status + the GitHub
  // device flow) registered later in registerApiRoutes. To avoid shadowing
  // them, explicitly fall through (next()) for those Pipeline Engine-owned paths so the
  ...
  if (webAuth) {
    ...
    // which /api/auth/* paths fall through to Pipeline Engine's own handlers vs. Better
    // Auth. A guard test asserts every Pipeline Engine-registered /api/auth/* route is in
  ...
  // of Pipeline Engine must NOT forward `/internal/*`. The startup guard below refuses
  ...
  if (!opts.skipPlatformAdapters && process.env.TELEGRAM_BOT_TOKEN) {
    ...
    telegramAdapter.onMessage(
      async ({ conversationId, message, userId: telegramUserId, displayName }) => {
        // Resolve Telegram user id (numeric) → Pipeline Engine user UUID.
