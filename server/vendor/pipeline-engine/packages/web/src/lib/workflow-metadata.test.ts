...
NOT for: Feature requests, enhancements, or non-bug work. Only for bugs/problems.`;
  ...
  test('handles "Handles:" and "Capability:" fallbacks (pipeline-assist style)', () => {
...
describe('getWorkflowDisplayName', () => {
  test('strips pipeline- prefix and converts to title case', () => {
    expect(getWorkflowDisplayName('pipeline-create-issue')).toBe('Create Issue');
    expect(getWorkflowDisplayName('pipeline-feature-development')).toBe('Feature Development');
  });

  test('preserves known acronyms (PR, CI, DAG)', () => {
    expect(getWorkflowDisplayName('pipeline-comprehensive-pr-review')).toBe(
      'Comprehensive PR Review'
    );
    expect(getWorkflowDisplayName('pipeline-ralph-dag')).toBe('Ralph DAG');
    expect(getWorkflowDisplayName('pipeline-interactive-prd')).toBe('Interactive PRD');
  });

  test('handles names without pipeline- prefix', () => {
    expect(getWorkflowDisplayName('my-custom-workflow')).toBe('My Custom Workflow');
  });

  test('handles single-word names', () => {
    expect(getWorkflowDisplayName('pipeline-assist')).toBe('Assist');
  });
});

describe('getWorkflowCategory', () => {
  test('categorizes review workflows', () => {
    expect(getWorkflowCategory('pipeline-comprehensive-pr-review', 'Review a PR')).toBe(
      'Code Review'
    );
    expect(getWorkflowCategory('pipeline-smart-pr-review', 'Smart PR review')).toBe('Code Review');
  });

  test('categorizes automation workflows', () => {
    expect(getWorkflowCategory('pipeline-create-issue', 'Create GitHub issue')).toBe('Automation');
    expect(getWorkflowCategory('pipeline-ralph-dag', 'Ralph implementation loop')).toBe('Automation');
    expect(getWorkflowCategory('pipeline-refactor-safely', 'Refactor code safely')).toBe(
      'Automation'
    );
  });

  test('categorizes CI/CD workflows', () => {
    expect(getWorkflowCategory('pipeline-validate-pr', 'Validate PR checks')).toBe('CI/CD');
    expect(getWorkflowCategory('pipeline-test-loop-dag', 'Run test loop')).toBe('CI/CD');
  });

  test('does not miscategorize workflows with "ci" as substring', () => {
    expect(getWorkflowCategory('pipeline-decision-tree', 'Routes decisions')).toBe('Development');
  ...
  test('categorizes development workflows', () => {
    expect(getWorkflowCategory('pipeline-feature-development', 'Implement a feature')).toBe(
      'Development'
    );
    expect(getWorkflowCategory('pipeline-assist', 'General help')).toBe('Development');
    expect(getWorkflowCategory('pipeline-idea-to-pr', 'From idea to PR')).toBe('Development');
  });
});
...
describe('getWorkflowTags', () => {
  test('derives tags from name and description', () => {
    ...
    const tags = getWorkflowTags('pipeline-comprehensive-pr-review', parsed);
  ...
  test('deduplicates tags', () => {
    ...
    const tags = getWorkflowTags('pipeline-pr-review', parsed);
  ...
  test('falls back to inference when no explicit tags', () => {
    ...
    const tags = getWorkflowTags('pipeline-pr-review', parsed, undefined);
    expect(tags).toContain('GitHub');
    expect(tags).toContain('Review');
  });
...
describe('getWorkflowIconName', () => {
  test('maps issue/bug workflows to Bug icon', () => {
    expect(getWorkflowIconName('pipeline-create-issue', 'Automation')).toBe('Bug');
    expect(getWorkflowIconName('pipeline-fix-github-issue', 'Automation')).toBe('Bug');
  });

  test('maps review workflows to Eye icon', () => {
    expect(getWorkflowIconName('pipeline-comprehensive-pr-review', 'Code Review')).toBe('Eye');
  });

  test('maps conflict workflows to GitMerge icon', () => {
    expect(getWorkflowIconName('pipeline-resolve-conflicts', 'Automation')).toBe('GitMerge');
  });

  test('maps feature workflows to Rocket icon', () => {
    expect(getWorkflowIconName('pipeline-feature-development', 'Development')).toBe('Rocket');
  });

  test('maps ralph to Bot icon', () => {
    expect(getWorkflowIconName('pipeline-ralph-dag', 'Automation')).toBe('Bot');
  });
