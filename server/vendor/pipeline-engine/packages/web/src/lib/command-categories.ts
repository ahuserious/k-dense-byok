...
/** Prefix-to-category mapping. Checked after stripping the `pipeline-` prefix. */
...
function stripPipelinePrefix(name: string): string {
  return name.startsWith('pipeline-') ? name.slice('pipeline-'.length) : name;
}

function findCategory(name: string): string {
  const stripped = stripPipelinePrefix(name);
