...
done before in this project's Pipeline Engine chats) should cut the time and cognitive
...
common denominator across Pipeline Engine's providers, and most of the plumbing already
...
- A genuinely non-`mcp__`-named path would mean _not_ riding the SDK loop —
  running an Pipeline Engine-owned tool loop against the raw API. Bigger divergence,
  rejected for speed.

> **Pi context:** Pi is slated to become Pipeline Engine's **main runner ~June 2026**.
...
`customTools`) run the handler in the Pipeline Engine process directly; Codex/OpenCode
...
Implement `manage_run`'s handler **once**, calling Pipeline Engine's existing
...
1. **Wave 1 — Claude + Pi (in-process handlers).** These are current-primary
   ...
   Pipeline Engine process — no transport.
   - Claude: `createSdkMcpServer()` + `tool()` (Zod shape); Pipeline Engine already
...
1. ~~In-process SDK tool vs. Pipeline Engine-owned loop~~ → **resolved: provider-neutral
...
4. Where the manage-run tool dispatches — confirm `operations/workflow-operations`
   ...
   for Claude; the served-MCP variant for Codex/OpenCode runs in the Pipeline Engine
...
- Artifacts — `api.ts:2768-2964`, `getRunArtifactsPath` (`packages/paths/src/pipeline-paths.ts:366-368`), console `ArtifactPanel.tsx`
