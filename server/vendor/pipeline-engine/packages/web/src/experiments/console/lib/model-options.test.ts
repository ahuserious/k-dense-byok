...
describe('curatedOptionsForAgent', () => {
  ...
  test('claude list leads with Opus 4.8 1M (the default) and offers Fable 5', () => {
    expect(CLAUDE_MODEL_OPTIONS[0]?.value).toBe('claude-opus-4-8[1m]');
    expect(CLAUDE_MODEL_OPTIONS.some(o => o.value === 'claude-opus-4-8[1m]')).toBe(true);
    expect(CLAUDE_MODEL_OPTIONS.some(o => o.value === 'claude-fable-5')).toBe(true);
  });
});
