/**
 * Pipeline Engine-owned env loader — runs at every entry point AFTER stripCwdEnv().
 *
 * Loads env vars from two pipeline-owned locations and emits operator-facing log
 ...
 *
 * Logging rules:
 *   - Each `[pipeline-engine] loaded N keys from …` line prints only when N > 0 AND
 ...
 *   - Silent in the common case (no pipeline-owned env files present).
...
import { getArchonEnvPath, getRepoArchonEnvPath } from './pipeline-paths';
...
/**
 * Load pipeline-owned env files. Call once, immediately after
 * `@archon/paths/strip-cwd-env-boot` at each entry point.
 *
 * Both loads use `override: true` so:
 *   - `~/.archon/.env` wins over shell-inherited vars (pipeline engine intent wins).
...
export function loadArchonEnv(cwd: string = process.cwd()): void {
  ...
  if (existsSync(homePath)) {
    ...
    if (count > 0 && isVerboseBoot()) {
      process.stderr.write(`[pipeline-engine] loaded ${count} keys from ${displayPath(homePath)}\n`);
    }
  }
  ...
  if (existsSync(repoPath)) {
    ...
    if (count > 0 && isVerboseBoot()) {
      process.stderr.write(
        `[pipeline-engine] loaded ${count} keys from ${displayPath(repoPath)} (repo scope, overrides user scope)\n`
      );
    }
  }
}
