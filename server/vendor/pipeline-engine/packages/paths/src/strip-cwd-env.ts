 ...
 *    When `pipeline engine` is invoked from inside a target repo, that repo's env vars
 *    leak into the Pipeline Engine process. `override: true` in dotenv only fixes keys
 *    that exist in both files — keys that only appear in the target repo's .env
 *    survive unaffected. We strip them.
 *
 * 2. Nested Claude Code session markers: When pipeline engine is launched from inside a
 ...
 *    (see upstream issue #1097), so the only way to prevent the nested-session
...
export function stripCwdEnv(cwd: string = process.cwd()): void {
    ...
    if (result.error) {
      ...
      if (code !== 'ENOENT') {
        process.stderr.write(
          `[pipeline-engine] Warning: could not parse ${filepath} for CWD env stripping: ${result.error.message}\n`
        );
      }
  ...
  if (cwdKeys.size > 0) {
    process.stderr.write(
      `[pipeline-engine] stripped ${cwdKeys.size} keys from ${cwd} (${strippedFiles.join(', ')}) to prevent target repo env from leaking into Pipeline Engine processes\n`
    );
  }
  ...
  if (process.env.CLAUDECODE === '1' && !process.env.ARCHON_SUPPRESS_NESTED_CLAUDE_WARNING) {
    process.stderr.write(
      '\u26a0  Detected CLAUDECODE=1 \u2014 running inside a Claude Code session.\n' +
        ...
        '   Workaround: run `pipeline-engine serve` from a regular shell.\n' +
        '   Suppress: set ARCHON_SUPPRESS_NESTED_CLAUDE_WARNING=1\n' +
        '   Details: see the nested-session diagnostic log.\n'
    );
  }
