...
describe('pipeline-paths', () => {
  ...
  describe('getArchonHome', () => {
    ...
    test('returns ARCHON_HOME when set (local)', () => {
      delete process.env.WORKSPACE_PATH;
      delete process.env.ARCHON_DOCKER;
      process.env.ARCHON_HOME = '/custom/pipeline';
      expect(getArchonHome()).toBe('/custom/pipeline');
    });

    test('expands tilde in ARCHON_HOME', () => {
      delete process.env.WORKSPACE_PATH;
      delete process.env.ARCHON_DOCKER;
      process.env.ARCHON_HOME = '~/my-pipeline-engine';
      expect(getArchonHome()).toBe(join(homedir(), 'my-pipeline-engine'));
    });
  ...
  describe('getArchonWorkspacesPath', () => {
    ...
    test('uses ARCHON_HOME when set', () => {
      ...
      delete process.env.ARCHON_DOCKER;
      process.env.ARCHON_HOME = '/custom/pipeline';
      expect(getArchonWorkspacesPath()).toBe(join('/custom/pipeline', 'workspaces'));
    });
  });
  ...
  describe('getArchonWorktreesPath', () => {
    ...
    test('uses ARCHON_HOME when set', () => {
      ...
      delete process.env.ARCHON_DOCKER;
      process.env.ARCHON_HOME = '/custom/pipeline';
      expect(getArchonWorktreesPath()).toBe(join('/custom/pipeline', 'worktrees'));
    });
  });
  ...
  describe('getCommandFolderSearchPaths', () => {
    ...
    test('includes configured folder when provided', () => {
      const paths = getCommandFolderSearchPaths('.claude/commands/pipeline-engine');
      expect(paths).toEqual([
        '.archon/commands',
        '.archon/commands/defaults',
        '.claude/commands/pipeline-engine',
      ]);
    });
  ...
  describe('getHomeWorkflowsPath', () => {
    ...
    test('uses ARCHON_HOME when set', () => {
      delete process.env.ARCHON_DOCKER;
      process.env.ARCHON_HOME = '/custom/pipeline';
      expect(getHomeWorkflowsPath()).toBe(join('/custom/pipeline', 'workflows'));
    });
  ...
  describe('getHomeCommandsPath', () => {
    ...
    test('uses ARCHON_HOME when set', () => {
      delete process.env.ARCHON_DOCKER;
      process.env.ARCHON_HOME = '/custom/pipeline';
      expect(getHomeCommandsPath()).toBe(join('/custom/pipeline', 'commands'));
    });
  });
  ...
  describe('getHomeScriptsPath', () => {
    ...
    test('uses ARCHON_HOME when set', () => {
      delete process.env.ARCHON_DOCKER;
      process.env.ARCHON_HOME = '/custom/pipeline';
      expect(getHomeScriptsPath()).toBe(join('/custom/pipeline', 'scripts'));
    });
  });
  ...
  describe('getLegacyHomeWorkflowsPath', () => {
    ...
    test('honors ARCHON_HOME so migration detection works in custom setups', () => {
      delete process.env.ARCHON_DOCKER;
      process.env.ARCHON_HOME = '/custom/pipeline';
      expect(getLegacyHomeWorkflowsPath()).toBe(join('/custom/pipeline', '.archon', 'workflows'));
    });
  });

  describe('getAppPipelineEngineBasePath', () => {
    test('returns repo root .archon path in local development', () => {
      delete process.env.ARCHON_DOCKER;
      delete process.env.WORKSPACE_PATH;
      const path = getAppPipelineEngineBasePath();
  ...
  describe('getDefaultCommandsPath', () => {
    test('returns commands/defaults under app pipeline engine base', () => {
      delete process.env.ARCHON_DOCKER;
      delete process.env.WORKSPACE_PATH;
  ...
  describe('getDefaultWorkflowsPath', () => {
    test('returns workflows/defaults under app pipeline engine base', () => {
      delete process.env.ARCHON_DOCKER;
      delete process.env.WORKSPACE_PATH;
  ...
  describe('getProjectRoot', () => {
    ...
    test('respects ARCHON_HOME', () => {
      ...
      expect(getProjectRoot('acme', 'widget')).toBe(
        join('/custom/pipeline', 'workspaces', 'acme', 'widget')
      );
    });
  ...
  describe('resolveProjectRootFromCwd', () => {
    ...
    test('works with ARCHON_HOME override', () => {
      delete process.env.WORKSPACE_PATH;
      delete process.env.ARCHON_DOCKER;
      process.env.ARCHON_HOME = join('/', 'custom', 'pipeline-engine');
      const cwd = join('/', 'custom', 'pipeline-engine', 'workspaces', 'acme', 'widget', 'source');
      expect(resolveProjectRootFromCwd(cwd)).toBe(
        join('/', 'custom', 'pipeline-engine', 'workspaces', 'acme', 'widget')
      );
    });
  });
});
...
describe('ensureProjectStructure', () => {
  ...
  beforeEach(async () => {
    ...
    tempArchonHome = join(
      tmpdir(),
      `pipeline-paths-test-${Date.now()}-${Math.random().toString(36).slice(2)}`
    );
...
describe('createProjectSourceSymlink', () => {
  ...
  beforeEach(async () => {
    ...
    tempArchonHome = join(
      tmpdir(),
      `pipeline-symlink-test-${Date.now()}-${Math.random().toString(36).slice(2)}`
    );
    process.env.ARCHON_HOME = tempArchonHome;

    tempTarget = join(
      tmpdir(),
      `pipeline-target-${Date.now()}-${Math.random().toString(36).slice(2)}`
    );
...
describe.skipIf(isWindows)('findMarkdownFilesRecursive - symlinks', () => {
  ...
  beforeEach(async () => {
    tempDir = join(
      tmpdir(),
      `pipeline-md-symlink-test-${Date.now()}-${Math.random().toString(36).slice(2)}`
    );
    sourceDir = join(
      tmpdir(),
      `pipeline-md-symlink-source-${Date.now()}-${Math.random().toString(36).slice(2)}`
    );
