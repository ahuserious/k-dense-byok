...
describe('getTelemetryStatus', () => {
  ...
  beforeEach(() => {
    saved = saveEnv();
    tmpHome = mkdtempSync(join(tmpdir(), 'pipeline-telemetry-status-'));
...
describe('resetTelemetryId', () => {
  ...
  beforeEach(() => {
    saved = saveEnv();
    tmpHome = mkdtempSync(join(tmpdir(), 'pipeline-telemetry-reset-'));
    process.env.ARCHON_HOME = tmpHome;
    resetTelemetryForTests();
  });
...
describe('first-run notice (via captureWorkflowInvoked)', () => {
  ...
  beforeEach(() => {
    saved = saveEnv();
    tmpHome = mkdtempSync(join(tmpdir(), 'pipeline-telemetry-notice-'));
    process.env.ARCHON_HOME = tmpHome;
...
describe('telemetry ID persistence', () => {
  ...
  beforeEach(() => {
    saved = saveEnv();
    tmpHome = mkdtempSync(join(tmpdir(), 'pipeline-telemetry-test-'));
    process.env.ARCHON_HOME = tmpHome;
...
describe('new capture functions are fire-and-forget no-throw', () => {
  ...
  beforeEach(() => {
    saved = saveEnv();
    tmpHome = mkdtempSync(join(tmpdir(), 'pipeline-telemetry-capture-'));
    process.env.ARCHON_HOME = tmpHome;
    resetTelemetryForTests();
  });
