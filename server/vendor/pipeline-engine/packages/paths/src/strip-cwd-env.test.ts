...
describe('stripCwdEnv — operator logging (#1302)', () => {
  ...
  it('emits [pipeline-engine] stripped line with count and filename when keys are stripped', () => {
    ...
    stripCwdEnv(tmpDir);
    const line = stderrWrites.find(s => s.startsWith('[pipeline-engine] stripped'));
  ...
  it('emits no [pipeline-engine] stripped line when no CWD .env files exist', () => {
    stripCwdEnv(tmpDir);
    const line = stderrWrites.find(s => s.startsWith('[pipeline-engine] stripped'));
    expect(line).toBeUndefined();
  });

  it('emits no [pipeline-engine] stripped line when .env file is empty', () => {
