...
const archonHomeDir = join(tmpRoot, 'pipeline-home');
...
let originalPipelineEngineVerboseBoot: string | undefined;
...
beforeEach(() => {
  ...
  originalPipelineEngineVerboseBoot = process.env.ARCHON_VERBOSE_BOOT;
...
afterEach(() => {
  ...
  if (originalPipelineEngineVerboseBoot === undefined) delete process.env.ARCHON_VERBOSE_BOOT;
  else process.env.ARCHON_VERBOSE_BOOT = originalPipelineEngineVerboseBoot;
...
describe('loadArchonEnv', () => {
  it('loads keys from ~/.archon/.env and emits a [pipeline-engine] loaded line when verbose-boot is set', () => {
    ...
    // the pipeline-home tmpdir segment rather than a literal `~` prefix.
    const line = stderrWrites.find(s => s.includes('[pipeline-engine] loaded') && !s.includes('repo scope'));
    expect(line).toBeDefined();
    expect(line).toContain('loaded 2 keys');
    expect(line).toContain(join('pipeline-home', '.env'));
  });
  ...
  it('does not emit loaded lines by default even when keys are present', () => {
    ...
    expect(process.env.TEST_EL_HOME_ONLY).toBe('from-home');
    expect(process.env.TEST_EL_REPO_ONLY).toBe('from-repo');
    const anyLoaded = stderrWrites.find(s => s.includes('[pipeline-engine] loaded'));
