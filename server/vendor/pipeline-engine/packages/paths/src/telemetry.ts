/**
 * Anonymous PostHog telemetry for Pipeline Engine.
 *
 ...
 * never crash Pipeline Engine.
 */
import { randomUUID } from 'crypto';
import { existsSync, mkdirSync, readFileSync, writeFileSync } from 'fs';
import { join } from 'path';
import type { PostHog } from 'posthog-node';
import { getArchonHome } from './pipeline-paths';
...
function collectMachineProperties(): Record<string, string | boolean> {
  ...
  return {
    os: process.platform,
    arch: process.arch,
    pipeline_engine_version: BUNDLED_VERSION,
...
/**
 * Apply the workflow-name privacy rule: bundled (Pipeline Engine-authored) workflows
 ...
 * distinct ID, host, and key source. Used by `pipeline-engine telemetry status` and
 * `pipeline-engine doctor` to surface what's happening without duplicating logic.
 */
 ...
 * `pipeline-engine telemetry reset`. Caller is responsible for any UX around it.
 *
...
function maybeShowFirstRunNotice(): void {
  ...
  const message =
    'Pipeline Engine collects anonymous usage telemetry — now also chat activity\n' +
 ...
 * (network, SDK, malformed props) at `debug` — telemetry must never crash Pipeline Engine.
