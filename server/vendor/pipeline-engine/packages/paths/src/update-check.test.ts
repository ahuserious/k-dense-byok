...
describe('parseLatestRelease', () => {
  test('parses valid response with v prefix', () => {
    const result = parseLatestRelease({
      tag_name: 'v0.4.0',
      html_url: 'https://github.com/K-Dense-AI/k-dense-byok/releases/tag/v0.4.0',
    });
    expect(result).toEqual({
      version: '0.4.0',
      url: 'https://github.com/K-Dense-AI/k-dense-byok/releases/tag/v0.4.0',
    });
  });
...
describe('checkForUpdate', () => {
  const testDir = join(tmpdir(), `pipeline-update-check-test-${Date.now()}`);
  ...
  test('returns result from fresh cache without fetching', async () => {
    const cache = {
      latestVersion: '0.5.0',
      releaseUrl: 'https://github.com/K-Dense-AI/k-dense-byok/releases/tag/v0.5.0',
      checkedAt: Date.now(),
    };
  ...
  test('fetches from GitHub when no cache exists', async () => {
    const fetchSpy = spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(
        JSON.stringify({
          tag_name: 'v0.5.0',
          html_url: 'https://github.com/K-Dense-AI/k-dense-byok/releases/tag/v0.5.0',
        }),
        { status: 200 }
      )
    );
    ...
    expect(cacheRaw.releaseUrl).toBe('https://github.com/K-Dense-AI/k-dense-byok/releases/tag/v0.5.0');
  ...
  test('returns updateAvailable: false when current matches latest', async () => {
    const cache = {
      latestVersion: '0.4.0',
      releaseUrl: 'https://github.com/K-Dense-AI/k-dense-byok/releases/tag/v0.4.0',
      checkedAt: Date.now(),
    };
    writeFileSync(join(testDir, 'update-check.json'), JSON.stringify(cache));
...
describe('getCachedUpdateCheck', () => {
  const testDir = join(tmpdir(), `pipeline-cached-check-test-${Date.now()}`);
  let originalArchonHome: string | undefined;
