/**
 * Pipeline Engine path resolution utilities
 *
 * Directory structure:
...
function getLog(): ReturnType<typeof createLogger> {
  if (!cachedLog) cachedLog = createLogger('pipeline-paths');
  return cachedLog;
}
...
/**
 * Get the Pipeline Engine home directory
 * - Docker: /.archon
 ...
 * deprecation warning pointing at the migration command. Pipeline Engine no longer
...
/**
 * Get the home-scope pipeline engine env file path (~/.archon/.env).
 * This is the pipeline-owned env location loaded by every entry point.
 */
...
/**
 * Get the repo-scope pipeline engine env file path (<cwd>/.archon/.env).
 * This is the pipeline-owned env location loaded with override: true AFTER the home
 ...
 * boot by stripCwdEnv() and never loaded by Pipeline Engine.
 */
 ...
 */
export function getAppPipelineEngineBasePath(): string {
  // This file is at packages/paths/src/pipeline-paths.ts
...
export function getDefaultCommandsPath(): string {
  return join(getAppPipelineEngineBasePath(), 'commands', 'defaults');
}
...
export function getDefaultWorkflowsPath(): string {
  return join(getAppPipelineEngineBasePath(), 'workflows', 'defaults');
}
...
/**
 * Log the Pipeline Engine paths configuration (for startup)
 */
