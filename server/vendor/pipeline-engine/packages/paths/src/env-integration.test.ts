...
describe('env isolation integration', () => {
  ...
  const engineDir = join(import.meta.dir, '__env-integration-pipeline-engine__');
  ...
  function simulateEntryPointFlow(cwdEnv: string, archonEnv: string): NodeJS.ProcessEnv {
    ...
    // Step 3: Load ~/.archon/.env with override — user's Pipeline Engine config wins
    // over any shell-inherited vars (same as real entry point).
    writeFileSync(join(engineDir, '.env'), archonEnv);
    config({ path: join(engineDir, '.env'), override: true });
  ...
  it('scenario 2: user has OAuth token in pipeline engine env + random key in CWD .env — CWD stripped, pipeline engine kept', () => {
    const subprocessEnv = simulateEntryPointFlow(
    ...
    // Pipeline Engine keys must be present
  ...
  it('scenario 4: same key in both CWD and pipeline engine env — pipeline engine value wins', () => {
    // User has ANTHROPIC_API_KEY in both places. CWD one is the target repo's,
    // pipeline engine one is the user's intentional config. Pipeline Engine must win.
    const subprocessEnv = simulateEntryPointFlow(
      'ANTHROPIC_API_KEY=sk-target-repo-WRONG\nSHARED_KEY=cwd-value\n',
      'ANTHROPIC_API_KEY=sk-my-real-key\nSHARED_KEY=pipeline-value\n'
    );

    // Pipeline Engine value wins (CWD was stripped, then pipeline engine loaded)
    expect(subprocessEnv.ANTHROPIC_API_KEY).toBe('sk-my-real-key');
    expect(subprocessEnv.SHARED_KEY).toBe('pipeline-value');
  });
  ...
  it('scenario 5: DATABASE_URL in CWD .env does not reach Pipeline Engine — pipeline engine uses its own DB', () => {
    // Target repo has DATABASE_URL for its own PostgreSQL. Pipeline Engine must NOT
    ...
    // CWD DATABASE_URL is stripped, pipeline engine's wins
  ...
  it('scenario 6: DATABASE_URL in CWD .env only (no pipeline engine env) — stripped entirely', () => {
  ...
  it('CLAUDE_CODE_OAUTH_TOKEN from pipeline engine env survives marker strip', () => {
