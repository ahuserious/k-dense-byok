 ...
 * provider `login()` for anthropic/github-copilot, or by Pipeline Engine's own OpenAI
 ...
 * the Pipeline Engine OpenAI refresh, and the only field-level parsing is
 ...
 *
 * The blob comes from Pipeline Engine's own OpenAI PKCE flow (`openai-oauth.ts`, #1924)
...
export function buildPiAuthJson(
  ...
  for (const { provider, cred } of creds) {
    ...
    // NOTE: the `openai` oauth blob carries an extra `id_token` field (Pipeline Engine's
