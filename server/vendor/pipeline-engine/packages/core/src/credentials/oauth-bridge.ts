 ...
 *
 *   - **manual-code** (Anthropic via Pi; OpenAI/ChatGPT via the Pipeline Engine-owned
...
/**
 * The Pipeline Engine-owned ChatGPT/Codex manual login (#1924): build the authorize
 ...
 * Pi's `login()` for anthropic/github-copilot, the Pipeline Engine-owned PKCE flow for
...
export async function startOAuth(userId: string, providerId: string): Promise<StartOAuthResult> {
  ...
  // `openai` (ChatGPT/Codex) runs the Pipeline Engine-OWNED PKCE flow (openai-oauth.ts)
