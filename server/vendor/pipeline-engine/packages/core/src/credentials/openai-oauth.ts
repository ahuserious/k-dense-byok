/**
 * Pipeline Engine-OWNED OpenAI (ChatGPT/Codex subscription) OAuth flow (#1924).
 *
 ...
 * empty one. Owning the exchange (and refresh) lets Pipeline Engine capture and
 ...
 * Pipeline Engine (see header) — the user copies the final redirect URL (or just the
...
/**
 * The OpenAI subscription credential blob Pipeline Engine stores. Superset of Pi's
...
async function postTokenRequest(
  ...
  try {
    ...
    // as a labeled error, not a raw SyntaxError mistaken for an Pipeline Engine bug.
    throw new Error(
