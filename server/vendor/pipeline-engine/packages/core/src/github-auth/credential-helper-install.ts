 ...
 *
 * Flow:
 *   1. Copy `scripts/git-credential-pipeline.sh` to `~/.archon/bin/` (idempotent;
 *      copy only on first call).
 *   2. Register the helper on the worktree's git config:
 *      `credential.https://github.com.helper = ~/.archon/bin/git-credential-pipeline`
 *
...
/** Repo-root → scripts/git-credential-pipeline.sh, resolved relative to this file. */
function sourceScriptPath(): string {
  // packages/core/src/github-auth/credential-helper-install.ts
  // ↑ ../../../..                                       repo root
  return resolve(import.meta.dir, '..', '..', '..', '..', 'scripts', 'git-credential-pipeline.sh');
}
...
export async function installCredentialHelper(
  ...
  const helperPath = resolve(binDir, 'git-credential-pipeline');
  try {
