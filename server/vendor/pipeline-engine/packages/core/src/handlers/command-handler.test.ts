  ...
  describe('handleCommand', () => {
    ...
    describe('/help', () => {
      test('should return help message', async () => {
        ...
        expect(result.message).toContain('Pipeline Engine Orchestrator');
    ...
    describe('/workflow run with load errors', () => {
      ...
      test('should match workflow name via suffix match', async () => {
        spyDiscoverWorkflows.mockResolvedValueOnce({
          workflows: [
            makeTestWorkflowWithSource({ name: 'pipeline-assist', description: 'General assistant' }),
          ],
          errors: [],
        });
        ...
        expect(result.success).toBe(true);
        expect(result.workflow?.definition.name).toBe('pipeline-assist');
      });
      ...
      test('should match workflow name via substring match', async () => {
        ...
        expect(result.success).toBe(true);
        expect(result.workflow?.definition.name).toBe('pipeline-smart-pr-review');
      });

      test('should return failure with candidates on ambiguous suffix match', async () => {
        spyDiscoverWorkflows.mockResolvedValueOnce({
          workflows: [
            makeTestWorkflowWithSource({ name: 'pipeline-review', description: 'Review' }),
        ...
        expect(result.message).toContain('pipeline-review');
