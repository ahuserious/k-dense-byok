...
`);
      ...
      expect(config.recommendedWorkflows).toEqual(['pipeline-plan', 'pipeline-fix-github-issue']);
    });

    test('coerces non-array recommendedWorkflows to undefined without throwing', async () => {
      mockFsReadFile.mockResolvedValue(`
recommendedWorkflows: "pipeline-plan"
`);
...
`;
    ...
    test('paths use pipeline-engine defaults', async () => {
      const error = new Error('ENOENT') as NodeJS.ErrnoException;
      error.code = 'ENOENT';
      mockFsReadFile.mockRejectedValue(error);
