...
const mockLoadConfig = mock(() =>
  Promise.resolve({
    ...
    streaming: { discord: 'batch', slack: 'batch' },
...
// Worktree sync mock
const mockSyncPipelineEngineToWorktree = mock(() => Promise.resolve(false));

mock.module('../utils/worktree-sync', () => ({
  syncPipelineEngineToWorktree: mockSyncPipelineEngineToWorktree,
}));
...
const testWorkflowDefs = makeTestWorkflowList(['fix-bug', 'add-feature', 'pipeline-assist']);
...
function clearAllMocks(): void {
  ...
  mockSyncPipelineEngineToWorktree.mockClear();
...
describe('parseOrchestratorCommands', () => {
  ...
  test('parses --prompt with double quotes', () => {
    const response =
      'I will analyze this.\n/invoke-workflow pipeline-assist --project test-project --prompt "Analyze the orchestrator module architecture"';
    const result = parseOrchestratorCommands(response, codebases, workflows);

    expect(result.workflowInvocation).not.toBeNull();
    expect(result.workflowInvocation?.workflowName).toBe('pipeline-assist');
    expect(result.workflowInvocation?.projectName).toBe('test-project');
  ...
  test('parses --prompt with spaces in the quoted value', () => {
    const response =
      '/invoke-workflow pipeline-assist --project test-project --prompt "Analyze the database schema and migration patterns in the project, focusing on table structure and relationships"';
    const result = parseOrchestratorCommands(response, codebases, workflows);
  ...
  test('parses --prompt with --project= equals syntax', () => {
    const response =
      '/invoke-workflow pipeline-assist --project=test-project --prompt "Summarize the README"';
    const result = parseOrchestratorCommands(response, codebases, workflows);
...
describe('orchestrator-agent handleMessage', () => {
  ...
  describe('workflow routing via AI', () => {
    ...
    test('passes synthesizedPrompt to workflow dispatch instead of original message', async () => {
      ...
      mockClient.sendQuery.mockImplementation(async function* () {
        yield {
          type: 'assistant',
          content: `Running analysis.\n/invoke-workflow pipeline-assist --project test-project --prompt "${synthesized}"`,
        };
        yield { type: 'result', sessionId: 'session-id' };
      });
    ...
    test('sends error when workflow found in parsing but not in dispatch', async () => {
      ...
      mockClient.sendQuery.mockImplementation(async function* () {
        yield {
          type: 'assistant',
          content: '/invoke-workflow pipeline-assist --project test-project',
        };
        yield { type: 'result', sessionId: 'session-id' };
      });
      ...
      expect(platform.sendMessage).toHaveBeenCalledWith(
        'chat-456',
        expect.stringContaining('pipeline-assist')
      );
    });
  });
  ...
  describe('workflow discovery', () => {
    ...
    test('syncs .archon to worktree before repo workflow discovery', async () => {
      ...
      mockSyncPipelineEngineToWorktree.mockImplementation(async () => {
        callOrder.push('sync');
        return false;
      });
      ...
      expect(mockSyncPipelineEngineToWorktree).toHaveBeenCalledWith('/workspace/project');
  ...
  describe('project registration', () => {
    ...
    test('/register-project stores detected current branch', async () => {
      const projectPath = await mkdtemp(join(tmpdir(), 'pipeline-register-project-'));
      try {
        ...
        await Bun.spawn(['git', 'commit', '--allow-empty', '-m', 'init'], {
          ...
          env: {
            ...process.env,
            GIT_AUTHOR_NAME: 'Pipeline Engine Test',
            GIT_AUTHOR_EMAIL: 'pipeline-test@example.com',
            GIT_COMMITTER_NAME: 'Pipeline Engine Test',
