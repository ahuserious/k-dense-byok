...
- If the user is asking a question or it's unclear whether they want a workflow → answer their question directly. You may suggest a workflow by name (e.g., "I can run the **pipeline-assist** workflow for this if you'd like"), but do NOT include /invoke-workflow in your response.

Example (clear intent):
I'll analyze the orchestrator module architecture for you.
/invoke-workflow pipeline-assist --project my-project --prompt "Analyze the orchestrator module architecture: explain how it routes messages, manages sessions, and dispatches workflows to AI clients"

Example (ambiguous — answer directly):
User: "What do you think about adding dark mode?"
Response: "Adding dark mode would involve... [answer the question]. If you'd like me to create a plan for this, I can run the **pipeline-idea-to-pr** workflow."
...
export function buildOrchestratorPrompt(
  ...
  let prompt = `# Pipeline Engine Orchestrator

You are Pipeline Engine, an intelligent coding assistant that manages multiple projects.
 ...
 * the `pipeline-engine` CLI over bash — the delivery of the `manage-run` skill for
 ...
 * inherits the same `pipeline-engine`-on-PATH convention the `pipeline-engine` skill already assumes.
 */
export function buildRunManagementSection(): string {
  return `## Managing Workflow Runs

You can inspect and control this project's workflow runs directly via the \`pipeline-engine\` CLI (bash) — you do NOT need to invoke a workflow for run management. Add \`--json\` to any command for a single clean, machine-readable line.
...
- \`pipeline-engine workflow runs [--json]\` — recent runs of ALL statuses for this project
- \`pipeline-engine workflow get <run-id> [--json]\` — one run's status/error (add \`--verbose\` for per-node detail)
- \`pipeline-engine workflow status [--json]\` — active runs only (running/paused)
- \`pipeline-engine workflow run <workflow> "<message>" --detach\` — start a run in the background (returns immediately)
- \`pipeline-engine workflow approve <run-id> [--json]\` / \`pipeline-engine workflow reject <run-id> [reason] [--json]\` — resolve a paused approval gate
- \`pipeline-engine workflow resume <run-id>\` — re-run a failed/paused run, skipping completed nodes (run as a background task; \`--json\` validates only)
- \`pipeline-engine workflow abandon <run-id> [--json]\` — cancel a non-terminal run
