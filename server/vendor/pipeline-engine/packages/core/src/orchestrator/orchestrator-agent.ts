...
import { syncPipelineEngineToWorktree } from '../utils/worktree-sync';
 ...
 * `pipeline-X` suffix convention workflows use.
 */
...
export function parseOrchestratorCommands(
  ...
  if (invokeMatch) {
    ...
    if (workflow) {
      ...
      // e.g., "Pipeline Engine" matches "research/pipeline-engine"
  ...
  if (conversation.codebase_id) {
    try {
      ...
      if (codebase) {
        ...
        await syncPipelineEngineToWorktree(workflowCwd);
...
export async function handleMessage(
  ...
  try {
      ...
      try {
        await platform.sendMessage(
          ...
          `ℹ️ Model tier 'large' isn't configured — using the '${chatRequest.matchedTier}' preset ` +
            ...
            'or `pipeline-engine ai tier set large <provider> <model>`.'
        );
      } catch (nudgeErr) {
        getLog().warn(
    ...
    // system-prompt pointer to the `pipeline-engine workflow …` CLI so they can still manage this
    ...
    // that needs `pipeline-engine` on PATH. Project-scoped only: the CLI commands require a
