...
describe('buildOrchestratorSystemAppend', () => {
  ...
  const workflows = [
    {
      ...
      nodes: [{ id: 'step1', command: 'pipeline-assist', depends_on: [] }],
    },
  ...
  test('returns orchestrator prompt when no codebase is scoped', () => {
    ...
    expect(result).toContain('# Pipeline Engine Orchestrator');
...
describe('buildRunManagementSection', () => {
  test('lists the run-management verbs and the --json hint', () => {
    ...
    for (const verb of [
      'pipeline-engine workflow runs',
      'pipeline-engine workflow get',
      'pipeline-engine workflow status',
      'pipeline-engine workflow run',
      'pipeline-engine workflow approve',
      'pipeline-engine workflow reject',
      'pipeline-engine workflow resume',
      'pipeline-engine workflow abandon',
    ]) {
