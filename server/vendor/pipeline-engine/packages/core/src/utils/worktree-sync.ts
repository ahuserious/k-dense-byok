 ...
 */
export async function syncPipelineEngineToWorktree(worktreePath: string): Promise<boolean> {
  try {
    ...
    const canonicalEnginePath = join(canonicalRepoPath, '.archon');
    const worktreeEnginePath = join(worktreePath, '.archon');

    // Canonical must exist; for worktree, ENOENT is expected (will be copied)
    const canonicalStat = await safeStat(canonicalEnginePath, 'canonical', false);
    if (!canonicalStat) {
      return false;
    }

    const worktreeStat = await safeStat(worktreeEnginePath, 'worktree', true);
