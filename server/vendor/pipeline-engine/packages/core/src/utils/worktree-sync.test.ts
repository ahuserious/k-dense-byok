...
import { syncPipelineEngineToWorktree } from './worktree-sync';

describe('syncPipelineEngineToWorktree', () => {
  ...
  test('returns false for non-worktree paths', async () => {
    ...
    const result = await syncPipelineEngineToWorktree('/regular/repo');
  ...
  test('returns false when canonical repo has no .archon', async () => {
    ...
    const result = await syncPipelineEngineToWorktree('/worktree/path');
