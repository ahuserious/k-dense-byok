...
// Default pipeline-engine workspaces path
...
describe('path-validation', () => {
  ...
  describe('isPathWithinWorkspace', () => {
    test('should allow paths within default pipeline-engine workspaces', async () => {
    ...
    test('should use ARCHON_HOME env var when set', async () => {
      ...
      const { isPathWithinWorkspace } = await importFresh();
      expect(isPathWithinWorkspace('/custom/pipeline/workspaces/repo')).toBe(true);
      const defaultPath = getDefaultWorkspacesPath();
  ...
  describe('validateAndResolvePath', () => {
    ...
    test('should use custom ARCHON_HOME for validation and error message', async () => {
      process.env.ARCHON_HOME = '/my/custom/pipeline';
      const { validateAndResolvePath } = await importFresh();
      const customWorkspace = resolve('/my/custom/pipeline/workspaces');
      // Valid path under custom workspace
      expect(validateAndResolvePath('/my/custom/pipeline/workspaces/repo')).toBe(
        resolve('/my/custom/pipeline/workspaces/repo')
      );
