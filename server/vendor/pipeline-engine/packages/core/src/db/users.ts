 ...
 *
 * Identity model:
 *   remote_agent_users          — Pipeline Engine-internal identity (one per human/bot)
...
/**
 * Resolve a platform-native user id to an Pipeline Engine User row, creating the user
...
/**
 * Raised when a GitHub account is already linked to a different Pipeline Engine user.
...
export class GithubIdentityConflictError extends Error {
  constructor(public readonly login: string) {
    super(`GitHub account @${login} is already linked to a different Pipeline Engine user.`);
...
/**
 * Attach a GitHub identity to an EXISTING Pipeline Engine user (the connecting actor).
