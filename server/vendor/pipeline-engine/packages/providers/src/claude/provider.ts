    ...
    if (err instanceof FirstEventTimeoutError) {
      ...
      throw new Error(
        'Claude Code subprocess produced no output within ' +
          ...
          'See the nested-session diagnostic log for details.'
      );
    }
    throw err;
  } finally {
 ...
 * operator pointed Pipeline Engine at a legacy Bun/Node-runnable cli script.
 * Otherwise return `false`.
 *
 * Safety: target-repo `.env` leaks are prevented by `stripCwdEnv()` in
 * `@archon/paths` (#1067), which deletes CWD `.env` keys from
 * `process.env` at every Pipeline Engine entry point before any subprocess is
...
/**
 * Normalize raw Claude SDK events into Pipeline Engine MessageChunks.
...
export class ClaudeProvider implements IAgentProvider {
  ...
  async *sendQuery(
    ...
    for (let attempt = 0; attempt <= MAX_SUBPROCESS_RETRIES; attempt++) {
      ...
      // 2b. Register in-process native tools (e.g. manage_run) as an pipeline engine MCP
