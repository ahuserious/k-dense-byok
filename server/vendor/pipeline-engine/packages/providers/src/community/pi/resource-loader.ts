 ...
 * Pipeline Engine to pi-ai ^0.67.5.
 *
...
export interface NoopResourceLoaderOptions {
   ...
   *
   * Used by the Pi provider to thread Pipeline Engine's name-based `skills:` node
   ...
   *
   * Trust boundary: enabling this loads arbitrary JS code with the Pipeline Engine
 ...
 * Pipeline Engine is the source of truth for skills, prompts, themes, and context
 ...
 * are still driven by Pipeline Engine's explicit `additionalSkillPaths` plumbing.
 *
 ...
 * setup or `session_start` handler. That state is never torn down between Pipeline Engine
 * `sendQuery()` calls: Pipeline Engine disposes sessions via `session.dispose()`, which —
