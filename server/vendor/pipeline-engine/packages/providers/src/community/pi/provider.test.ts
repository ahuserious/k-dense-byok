...
describe('PiProvider', () => {
  ...
  test('sendQuery installs PI_PACKAGE_DIR shim before Pi SDK loads', async () => {
    ...
    // pipeline engine binaries. The shim writes a stub package.json to tmpdir and sets
    ...
    expect(process.env.PI_PACKAGE_DIR).toContain('pipeline-pi-shim');
    ...
    expect(mockLogger.info).toHaveBeenCalledWith(
      {
        ...
        envHint: expect.stringContaining("not in the Pipeline Engine adapter's env-var table"),
  ...
  test('result chunk carries Pi sessionId (for Pipeline Engine to store and reuse)', async () => {
    process.env.GEMINI_API_KEY = 'sk-test';
    resetScript(scriptedAgentEnd());
