...
/**
 * Extract Pipeline Engine TokenUsage from Pi's Usage struct.
...
/**
 * Pure mapper from Pi's `AgentSessionEvent` → zero-or-more Pipeline Engine `MessageChunk`s.
 *
 ...
 *  - compaction_start / compaction_end (auto-compaction opaque to Pipeline Engine)
...
/**
 * Bridge a Pi `AgentSession` into Pipeline Engine's `AsyncGenerator<MessageChunk>` contract.
 *
 * Behavior:
...
export async function* bridgeSession(
  ...
  try {
    for await (const item of queue) {
      ...
      // Annotate the terminal result chunk with Pi's session UUID so Pipeline Engine's
