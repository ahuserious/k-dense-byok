...
// compiled Pipeline Engine binary `getPackageJsonPath()` resolves to
// `dirname(process.execPath) + "/package.json"` — a path that doesn't exist —
// and pipeline engine crashes at startup before any command runs (v0.3.7 symptom).
//
 ...
 * (which fails inside a compiled pipeline engine binary). Pi only reads three
...
export function ensurePiPackageDirShim(): void {
  const shimDir = join(tmpdir(), 'pipeline-pi-shim');
...
// ["ANTHROPIC_OAUTH_TOKEN", "ANTHROPIC_API_KEY"]). Pipeline Engine delivers subscriptions
...
export class PiProvider implements IAgentProvider {
  async *sendQuery(
    ...
    // mode (pipeline engine compiled binary crashes at startup when Pi's config.js
    ...
    try {
      // Pipeline Engine delivers per-user credentials (API keys + subscriptions) as a
        ...
        getLog().info(
          {
            ...
            envHint: `Provider '${parsed.provider}' is not in the Pipeline Engine adapter's env-var table — file an issue if you want a shortcut env var for it.`,
    ...
    // 4. Translate Pipeline Engine nodeConfig to Pi SDK options. All three translations
    ...
    //    4d. skills: Pipeline Engine uses name references (e.g. `skills: [agent-browser]`).
