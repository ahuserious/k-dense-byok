 ...
 * Pipeline Engine's common surface includes 'off' (from Codex's modelReasoningEffort)
 * and 'max' (from Claude's EffortLevel enum). Map into Pi's vocabulary:
 *  - 'off'    → undefined (no explicit thinkingLevel; Pi's implicit off)
 *  - 'max'    → 'xhigh'  (Pipeline Engine's EffortLevel doesn't have xhigh)
 *  - others pass through if they're already Pi-native
 *
 * See packages/workflows/src/schemas/dag-node.ts#effortLevelSchema for
 * the Pipeline Engine schema enum (`low | medium | high | max`). Workflow YAML can
 * only carry Pipeline Engine-enum values; Pi-native `minimal` / `xhigh` are accepted
...
/**
 * Resolve Pipeline Engine's `effort` / `thinking` node fields to Pi's `ThinkingLevel`.
 *
...
/**
 * Filter Pi's built-in tool set against Pipeline Engine's `allowed_tools` /
 ...
 *   - allowed_tools: [] → return [] (explicit no-tools; valid Pipeline Engine idiom)
