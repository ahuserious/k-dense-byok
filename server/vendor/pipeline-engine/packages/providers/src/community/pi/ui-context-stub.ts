...
/** Pushes UI notifications into Pipeline Engine's event stream. Set/cleared by bridgeSession. */
...
/**
 * Minimal ExtensionUIContext for Pipeline Engine's headless Pi sessions. Binding this
...
export function createArchonUIContext(bridge: ArchonUIBridge): ExtensionUIContext {
  ...
  return {
    ...
    setWorkingVisible(_visible: boolean): void {
      // No-op: Pipeline Engine has no built-in working-loader row. Added in pi 0.70.3.
      noop();
    },
    ...
    setTheme(_theme: string | Theme): { success: boolean; error?: string } {
      return { success: false, error: 'Theme switching not supported in Pipeline Engine remote UI stub' };
    },
