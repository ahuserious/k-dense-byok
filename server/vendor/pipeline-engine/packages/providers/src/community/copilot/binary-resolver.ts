/**
 * Copilot CLI binary resolver for compiled (bun --compile) pipeline engine binaries.
 *
 ...
 * via `import.meta.url`. In compiled pipeline engine binaries that path is frozen to
...
export async function resolveCopilotBinaryPath(
  ...
  throw new Error(
    'Copilot CLI binary not found. The Copilot provider requires the\n' +
      ...
      'compiled Pipeline Engine builds.\n\n' +
