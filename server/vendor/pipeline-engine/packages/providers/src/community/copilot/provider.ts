 ...
 * binary path + reasoning config, translates Pipeline Engine workflow options
...
/**
 * Resolve Copilot's `reasoningEffort` from Pipeline Engine's workflow inputs.
 * Precedence:
 *   nodeConfig.thinking > nodeConfig.effort > config.modelReasoningEffort
 *
 * Pipeline Engine's `effort` schema is `'low' | 'medium' | 'high' | 'max'` — we map
...
/**
 * Translate Pipeline Engine's per-node `allowed_tools` / `denied_tools` to Copilot's
...
/**
 * Translate Pipeline Engine's `nodeConfig.mcp` (JSON-file path) to Copilot's
...
/**
 * Translate Pipeline Engine's `nodeConfig.skills` (string names) to Copilot's
...
/**
 * Translate Pipeline Engine's `nodeConfig.agents` (Record<name, AgentDef>) to
 ...
 * Pipeline Engine agent fields Copilot cannot represent (`model`, `disallowedTools`,
 * `skills`, `maxTurns`) surface as one consolidated warning per agent.
 *
 * We do NOT set `SessionConfig.agent` — Pipeline Engine's workflow model invokes
 ...
 * `@github/copilot-sdk`, translating Pipeline Engine workflow options (tools, MCP,
...
export class CopilotProvider implements IAgentProvider {
  ...
  async *sendQuery(
    ...
    // Pipeline Engine's dag-executor sets `forkSession: true` on every reuse so retries
