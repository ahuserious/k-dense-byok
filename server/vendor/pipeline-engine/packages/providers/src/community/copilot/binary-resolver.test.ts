...
mock.module('@archon/paths', () => ({
  ...
  getArchonHome: mock(() => '/tmp/test-pipeline-home'),
}));
...
describe('resolveCopilotBinaryPath (binary mode)', () => {
  ...
  test('checks vendor directory when no env or config path', async () => {
    ...
    expect(normalized).toContain('/tmp/test-pipeline-home/vendor/copilot/');
  });
...
describe('isExecutableFile', () => {
  ...
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'pipeline-copilot-resolver-'));
