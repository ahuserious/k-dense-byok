...
export async function materializeAgents(
  ...
  // Remove stale pipeline-owned agent files that aren't in the current request
  const currentPipelineFiles = new Set(
    Object.keys(agents).map(key => `pipeline-${toKebabCase(key)}.md`)
  );
  try {
    const existing = await readdir(agentsDir);
    await Promise.all(
      existing
        .filter(f => f.startsWith('pipeline-') && !currentPipelineFiles.has(f))
  ...
  await Promise.all(
    Object.entries(agents).map(([key, config]) => {
      const filename = `pipeline-${toKebabCase(key)}.md`;
