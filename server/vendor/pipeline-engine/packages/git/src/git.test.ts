...
describe('git utilities', () => {
  ...
  describe('getWorktreeBase', () => {
    ...
    test('uses ARCHON_HOME for the workspace-scoped base (local non-Docker)', () => {
      ...
      const result = git.getWorktreeBase('/workspace/my-repo');
      expect(result).toEqual({
        base: join('/custom/pipeline', 'workspaces', 'workspace', 'my-repo', 'worktrees'),
        layout: 'workspace-scoped',
      });
    });

    test('uses the Docker pipeline-engine home for the workspace-scoped base', () => {
      delete process.env.ARCHON_HOME;
      process.env.ARCHON_DOCKER = 'true';
      const result = git.getWorktreeBase('/workspace/my-repo');
      expect(result).toEqual({
    ...
    test('workspace-scoped path honors ARCHON_HOME override', () => {
      delete process.env.WORKSPACE_PATH;
      delete process.env.ARCHON_DOCKER;
      process.env.ARCHON_HOME = join('/', 'custom', 'pipeline');
      const repoPath = join('/', 'custom', 'pipeline', 'workspaces', 'acme', 'widget', 'source');
      const result = git.getWorktreeBase(repoPath);
      expect(result).toEqual({
        base: join('/', 'custom', 'pipeline', 'workspaces', 'acme', 'widget', 'worktrees'),
        layout: 'workspace-scoped',
      });
    });
