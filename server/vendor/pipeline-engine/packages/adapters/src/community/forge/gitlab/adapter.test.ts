...
function createNotePayload(overrides?: {
  ...
  const base: Record<string, unknown> = {
    ...
    object_attributes: {
      ...
      note: overrides?.note ?? '@pipeline hello',
      noteable_id: 100,
    },
  };
...
describe('GitLabAdapter', () => {
  ...
  describe('self-filtering', () => {
    ...
    test('ignores comments from bot account', async () => {
      const adapter = createAdapter({ botMention: 'pipeline' });
      const payload = createNotePayload({
        note: '@pipeline something',
        username: 'pipeline',
      });
      await adapter.handleWebhook(payload, 'test-secret');
      expect(mockHandleMessage).not.toHaveBeenCalled();
  ...
  describe('authorization', () => {
    test('rejects unauthorized users when whitelist is set', async () => {
      ...
      const adapter = createAdapter();
      const payload = createNotePayload({ username: 'mallory', note: '@pipeline hello' });
      await adapter.handleWebhook(payload, 'test-secret');
      expect(mockHandleMessage).not.toHaveBeenCalled();
    });
  });
    ...
    test('ignores MR open events (descriptions are not commands)', async () => {
      ...
      const payload = JSON.stringify({
        ...
        object_attributes: {
          ...
          action: 'open',
          title: 'New MR',
          description: '@pipeline review this',
          state: 'opened',
          source_branch: 'feature',
          target_branch: 'main',
          source_project_id: 1,
  ...
  describe('mention patterns', () => {
    test('detects mention at end of string', async () => {
      const adapter = createAdapter();
      const payload = createNotePayload({ note: 'help me @pipeline' });
      await adapter.handleWebhook(payload, 'test-secret');
    ...
    test('detects mention with comma separator', async () => {
      const adapter = createAdapter();
      const payload = createNotePayload({ note: '@pipeline, please help' });
      await adapter.handleWebhook(payload, 'test-secret');
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.objectContaining({ conversationId: 'mygroup/myproject#1' }),
        'gitlab.webhook_setup_failed'
    ...
    test('case-insensitive mention detection', async () => {
      const adapter = createAdapter();
      const payload = createNotePayload({ note: '@PIPELINE help' });
      await adapter.handleWebhook(payload, 'test-secret');
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.objectContaining({ conversationId: 'mygroup/myproject#1' }),
        'gitlab.webhook_setup_failed'
  ...
  describe('stripMention', () => {
    ...
    test('strips mention followed by space', () => {
      expect(strip('@pipeline hello')).toBe('hello');
    });

    test('strips mention followed by comma', () => {
      expect(strip('@pipeline, please help')).toBe('please help');
    });

    test('strips mention at end of string', () => {
      expect(strip('help me @pipeline')).toBe('help me');
    });

    test('does NOT strip partial-prefix username (@pipelinebot)', () => {
      // Regression: old regex `@pipeline[\s,:;]*` would strip '@pipeline' from '@pipelinebot',
      // yielding 'bot hello'. New regex requires a separator or end-of-string.
      expect(strip('@pipelinebot hello')).toBe('@pipelinebot hello');
    });
  });
  ...
  describe('sendMessage', () => {
    ...
    test('posts to correct issue notes API endpoint', async () => {
      const adapter = createAdapter();
      await adapter.sendMessage('mygroup/myproject#42', 'Hello from Pipeline Engine');
    ...
    test('appends bot response marker to outgoing comments', async () => {
      ...
      expect(body.body).toContain('<!-- pipeline-bot-response -->');
