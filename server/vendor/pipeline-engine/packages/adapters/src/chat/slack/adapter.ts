...
export class SlackAdapter implements IPlatformAdapter {
  ...
  private async handleSlashCommand(
    ...
    if (!this.messageHandler) {
      await respond({
        ...
        text: 'Pipeline Engine is starting up — try again in a moment.',
      });
      return;
    }
    ...
    if (!raw) {
      const help =
        kind === 'pipeline-workflow'
          ? 'Usage: `/pipeline-workflow <subcommand>` — e.g. `list`, `status`, `run <name> <args>`, `approve <id>`, `reject <id> <reason>`, `abandon <id>`.'
          : 'Usage: `/pipeline <message>` — talk to Pipeline Engine in this channel.';
      await respond({ response_type: 'ephemeral', text: help });
      return;
    }

    // `/pipeline connect github` — device-flow GitHub connect, handled inline
    // (no orchestrator dispatch / seed message).
    if (kind === 'pipeline' && /^connect\s+github\b/i.test(raw)) {
      await this.handleConnectGithub(command, respond);
      return;
    }

    const messageText = kind === 'pipeline-workflow' ? `/workflow ${raw}` : raw;
    ...
    try {
      const seedText =
        kind === 'pipeline-workflow'
          ? `<@${actorId}> ran \`/pipeline-workflow ${raw}\``
          : `<@${actorId}> via /pipeline: ${raw}`;
  ...
  /**
   * Handle `/pipeline connect github`: resolve the invoking Slack user to an
   * Pipeline Engine user, then drive the device flow. The device code and final result
  ...
  private async handleConnectGithub(
    ...
    if (!isPerUserGitHubEnabled()) {
      await respond({
        response_type: 'ephemeral',
        text: 'GitHub connect is not enabled on this Pipeline Engine install (requires the GitHub App + token encryption).',
      });
      return;
    }
    ...
    try {
      ...
      await respond({
        response_type: 'ephemeral',
        text: 'Could not resolve your Pipeline Engine identity — try again in a moment.',
      });
      return;
    }
