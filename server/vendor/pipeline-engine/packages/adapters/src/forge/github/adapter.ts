...
const BOT_RESPONSE_MARKER = '<!-- pipeline-bot-response -->';
...
export class GitHubAdapter implements IPlatformAdapter {
  ...
  /**
   * conversationId → originating Pipeline Engine userId (the last human to trigger this
  ...
  private async ensureRepoReady(
    ...
    if (!cloneResult.ok) {
      ...
      if (cloneResult.error.code === 'not_a_repo') {
        ...
        const authHint =
          this.auth.kind === 'app'
            ? 'Check that the Pipeline Engine GitHub App is installed on the org and has the Contents:Read permission.'
...
${userComment}`;
  ...
  async handleWebhook(payload: string, signature: string): Promise<void> {
    ...
    // 5b. Resolve GitHub login → Pipeline Engine user (auto-create on first sight).
