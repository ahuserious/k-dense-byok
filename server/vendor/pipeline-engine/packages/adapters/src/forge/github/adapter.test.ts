...
describe('GitHubAdapter', () => {
  ...
  describe('bot mention detection', () => {
    ...
    test('should detect mention when it is the entire message', () => {
      ...
      expect(hasMention.call(adapterWithMention, '@Pipeline')).toBe(true);
      expect(hasMention.call(adapterWithMention, '@PIPELINE')).toBe(true);
      expect(hasMention.call(adapterWithMention, '@pipeline')).toBe(true);
    });
  ...
  describe('self-filtering', () => {
     ...
     */
    function createSelfFilterAdapter(botMention = 'pipeline'): GitHubAdapter {
    ...
    test('attribution falls back to sender.login when comment.user is absent', async () => {
      ...
      const payload = createCommentPayload('@pipeline help', undefined); // no comment.user
    ...
    test('attribution prefers comment.user.login over sender.login when both present', async () => {
      ...
      const payload = JSON.stringify({
        ...
        comment: { body: '@pipeline look at this', user: { login: 'reviewer-alice' } },
        repository: {
          owner: { login: 'testuser' },
          name: 'testrepo',
          full_name: 'testuser/testrepo',
    ...
    test('handleWebhook never throws when identity resolution fails', async () => {
      ...
      const payload = createCommentPayload('@pipeline help', 'user123');
    ...
    test('should ignore comments from the bot itself', async () => {
      const adapter = createSelfFilterAdapter();
      const payload = createCommentPayload('@pipeline fix this', 'pipeline');
    ...
    test('should handle case-insensitive username matching', async () => {
      const adapter = createSelfFilterAdapter('Pipeline Engine'); // Mixed case config
      const payload = createCommentPayload('@pipeline test', 'pipeline'); // Lowercase author
    ...
    test('should NOT filter comments from real users', async () => {
      const adapter = createSelfFilterAdapter();
      const payload = createCommentPayload('@pipeline please help', 'user123');
    ...
    test('should process comments without bot marker from same user', async () => {
      ...
      const payload = createCommentPayload('@pipeline fix this', 'Wirasm');
    ...
    test('should handle missing comment.user gracefully', async () => {
      const adapter = createSelfFilterAdapter();
      const payload = createCommentPayload('@pipeline help', undefined); // No user field
  ...
  describe('conversationId format', () => {
    ...
    test('postComment appends bot marker to outgoing comments', async () => {
      ...
      expect(body).toContain('<!-- pipeline-bot-response -->');
  ...
  describe('App mode', () => {
    ...
    test('webhook installation.id primes the lookup cache (no extra round trip)', async () => {
      ...
      const payload = JSON.stringify({
        ...
        comment: { body: '@pipeline help', user: { login: 'someone' } },
        repository: {
          owner: { login: 'alpha' },
          name: 'repo',
    ...
    test('self-filter ignores comments authored by "<slug>[bot]"', async () => {
      const { adapter } = createAppModeAdapter({ slug: 'pipeline-bot' });
      const payload = JSON.stringify({
        action: 'created',
        issue: {
          number: 1,
        ...
        comment: {
          body: '@pipeline ping', // would normally trigger; bot login filter must reject first
          user: { login: 'pipeline-bot[bot]' },
        },
        repository: {
          owner: { login: 'o' },
          name: 'r',
    ...
    test('AppNotInstalledError on clone surfaces a clean message', async () => {
      ...
      provider.getToken.mockImplementationOnce(async (owner: string, repo: string) => {
        throw new AppNotInstalledError(owner, repo, 'pipeline-test');
      });
      // @ts-expect-error - calling private method
