...
export class WorktreeProvider implements IIsolationProvider {
  ...
  generateBranchName(request: IsolationRequest): string {
    switch (request.workflowType) {
      case 'issue':
        return `pipeline/issue-${request.identifier}`;
      case 'pr':
        // Same-repo PRs use actual branch (already exists on remote), fork PRs use synthetic
        if (!request.isForkPR) {
          return request.prBranch;
        }
        return `pipeline/pr-${request.identifier}-review`;
      case 'review':
        return `pipeline/review-${request.identifier}`;
      case 'thread':
        // Use short hash for arbitrary thread IDs (Slack, Discord)
        return `pipeline/thread-${this.shortHash(request.identifier)}`;
      case 'task':
        return `pipeline/task-${this.slugify(request.identifier)}`;
    }
  }
  ...
  private async syncWorkspaceBeforeCreate(
    ...
    try {
      ...
      // Only hard-reset for Pipeline Engine-managed clones when creating isolated worktrees.
