...
describe('WorktreeProvider', () => {
  ...
  describe('generateBranchName', () => {
    test('generates issue-N for issue workflows', () => {
      ...
      expect(provider.generateBranchName(request)).toBe('pipeline/issue-42');
    });
    ...
    test('generates pr-N-review for fork PR workflows', () => {
      ...
      expect(provider.generateBranchName(request)).toBe('pipeline/pr-123-review');
    });
    ...
    test('generates review-N for review workflows', () => {
      ...
      expect(provider.generateBranchName(request)).toBe('pipeline/review-456');
    });
    ...
    test('generates thread-{hash} for thread workflows', () => {
      ...
      expect(name).toMatch(/^pipeline\/thread-[a-f0-9]{8}$/);
    });
    ...
    test('generates task-{slug} for task workflows', () => {
      ...
      expect(provider.generateBranchName(request)).toBe('pipeline/task-add-dark-mode');
    });
  ...
  describe('create', () => {
    ...
    test('creates worktree for issue workflow', async () => {
      ...
      expect(env.branchName).toBe('pipeline/issue-42');
    ...
    test('throws when branch already exists and fromBranch is specified', async () => {
      ...
      alreadyExistsError.stderr =
        "fatal: a branch named 'pipeline/task-test-adapters' already exists";
      ...
      await expect(provider.create(request)).rejects.toThrow(
        'Branch "pipeline/task-test-adapters" already exists. Cannot create it from "feature/extract-adapters".'
      );
    });
    ...
    test('resets and reuses existing branch when it already exists and no fromBranch', async () => {
      ...
      expect(execSpy).toHaveBeenCalledWith(
        'git',
        ['-C', '/workspace/repo', 'branch', '-f', 'pipeline/task-test-adapters', 'origin/main'],
        expect.any(Object)
      );
    ...
    test('adopts existing worktree when repo ownership matches', async () => {
      ...
      mockReadFile.mockResolvedValue('gitdir: /workspace/repo/.git/worktrees/pipeline/issue-42\n');
    ...
    test('throws when worktree belongs to different repo root (cross-checkout)', async () => {
      worktreeExistsSpy.mockResolvedValue(true);
      mockReadFile.mockResolvedValue('gitdir: /different/repo/.git/worktrees/pipeline/issue-42\n');
    ...
    test('resets stale branch to start-point when it already exists', async () => {
      ...
      execSpy.mockImplementation(async (_cmd: string, args: string[]) => {
        ...
        if (callCount === 1 && args.includes('-b')) {
          const error = new Error(
            'fatal: A branch named pipeline/issue-42 already exists.'
          ) as Error & {
            stderr?: string;
          };
          error.stderr = 'fatal: A branch named pipeline/issue-42 already exists.';
          throw error;
        }
      ...
      expect(execSpy).toHaveBeenCalledWith(
        'git',
        ['-C', '/workspace/repo', 'branch', '-f', 'pipeline/issue-42', 'origin/main'],
        expect.any(Object)
      );
    ...
    test('propagates error if branch -f reset fails (protected branch, etc.)', async () => {
      execSpy.mockImplementation(async (_cmd: string, args: string[]) => {
        ...
        if (args.includes('branch') && args.includes('-f')) {
          ...
          error.stderr = "fatal: cannot force update the current branch 'pipeline/issue-42'";
          throw error;
        }
        return { stdout: '', stderr: '' };
      });
      ...
      const secondWorktreeAdd = execSpy.mock.calls.filter((call: unknown[]) => {
        ...
        return (
          ...
          args.includes('add') &&
          !args.includes('-b') &&
          args.includes('pipeline/issue-42')
        );
      });
  ...
  describe('file copying', () => {
    ...
    test('does not copy files when adopting existing worktree', async () => {
      ...
      mockReadFile.mockResolvedValue(
        'gitdir: /.archon/workspaces/owner/repo/.git/worktrees/pipeline/issue-42\n'
      );
      const configLoader: RepoConfigLoader = async () => ({
  ...
  describe('orphan directory handling', () => {
    ...
    test('does not remove directory if it is a valid worktree', async () => {
      ...
      mockReadFile.mockResolvedValue('gitdir: /workspace/repo/.git/worktrees/pipeline/issue-999\n');
  ...
  describe('workspace sync before worktree creation', () => {
    ...
    test('does not sync workspace when adopting existing worktree', async () => {
      ...
      worktreeExistsSpy.mockResolvedValue(true);
      mockReadFile.mockResolvedValue(
        'gitdir: /workspace/owner/repo/.git/worktrees/pipeline/issue-42\n'
      );
