...
async function resolveNodeProviderAndModel(
  ...
  // silently overrides Pipeline Engine's skills wrapper. User wins (by design) but
  ...
  if (
    ...
    await safeSendMessage(
      platform,
      conversationId,
      `Warning: Node '${node.id}' defines an agent with reserved ID 'dag-node-skills' AND uses 'skills:'. Your inline agent overrides Pipeline Engine's automatic skills wrapper — the 'skills:' field will NOT take effect. Rename the agent or remove 'skills:' to fix.`,
      { workflowId: workflowRunId, nodeName: node.id }
    );
  }
  ...
  try {
    if (isInlineScript(finalScript)) {
      ...
      if (node.runtime === 'bun') {
        ...
        // subprocess despite Pipeline Engine's parent process cleanup.
