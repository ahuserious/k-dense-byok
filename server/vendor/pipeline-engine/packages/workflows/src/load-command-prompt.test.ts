...
describe('loadCommandPrompt — home-scope resolution', () => {
  ...
  beforeEach(() => {
    ...
    archonHome = mkdtempSync(join(tmpdir(), 'pipeline-home-'));
    repoRoot = mkdtempSync(join(tmpdir(), 'pipeline-repo-'));
  ...
  it('resolves a symlinked home command and reads target content', async () => {
    const sourceDir = mkdtempSync(join(tmpdir(), 'pipeline-command-source-'));
    try {
