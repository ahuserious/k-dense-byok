 ...
 * 3. Suffix match (e.g. "assist" → "pipeline-assist")
 * 4. Substring match (e.g. "smart" → "pipeline-smart-pr-review")
 *
...
export function resolveWorkflowName(
  ...
  return (
    ...
    // Tier 3: Suffix match (e.g. "assist" matches "pipeline-assist")
    checkTier(
      workflows.filter(w => w.name.toLowerCase().endsWith(`-${lowerName}`)),
      'workflow.resolve_suffix_match'
    ) ??
    // Tier 4: Substring match (e.g. "smart" matches "pipeline-smart-pr-review")
    checkTier(
