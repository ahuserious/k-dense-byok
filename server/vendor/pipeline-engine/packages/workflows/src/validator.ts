...
export async function validateWorkflowResources(
  ...
  for (const node of workflow.nodes) {
    ...
    // $node.output values are injected PRE-QUOTED by Pipeline Engine: small values are
    ...
    const warnDoubleQuoted = (body: string, field: string): void => {
      if (doubleQuotedOutputRef.test(body)) {
        issues.push({
          ...
          field,
          message:
            '`"$nodeId.output"` — double-quoting a substitution that is already shell-quoted by Pipeline Engine produces the wrong value',
