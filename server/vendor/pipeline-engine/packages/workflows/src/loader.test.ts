...
`;
  ...
  describe('multi-source loading', () => {
    it('ships no software-development app defaults when enabled', async () => {
    ...
    it('loads a repo workflow when app defaults are enabled', async () => {
...
description: My custom assist (overrides pipeline-assist)
nodes:
  - id: custom
    command: custom-command
`;
      await writeFile(join(repoWorkflowDir, 'pipeline-assist.yaml'), repoWorkflowYaml);

      const result = await discoverWorkflows(testDir, { loadDefaults: true });
      const workflows = result.workflows.map(ws => ws.workflow);

      const assistWorkflow = workflows.find(
        w => w.name === 'my-custom-assist' || w.name === 'pipeline-assist'
      );
      expect(assistWorkflow).toBeDefined();
      expect(assistWorkflow?.name).toBe('my-custom-assist');
      expect(assistWorkflow?.description).toBe('My custom assist (overrides pipeline-assist)');
    });
...
`;
  ...
  describe('discoverWorkflowsWithConfig', () => {
    ...
    it('should default to loadDefaults: true when config load fails', async () => {
      ...
      // The fallback still enables defaults, but this distribution ships none.
      expect(result.workflows).toHaveLength(0);
    });
  ...
  describe('binary build bundled workflows', () => {
    ...
    it('ships no bundled workflows when running as binary', async () => {
    ...
    it('loads repo workflows in binary mode', async () => {
      // Simulate binary build
      isBinaryBuildSpy.mockReturnValue(true);
...
description: Custom override of pipeline-assist
nodes:
  - id: custom
    command: custom
`;
      ...
      const assistWorkflow = workflows.find(
        w => w.name === 'custom-assist-override' || w.name === 'pipeline-assist'
      );
      expect(assistWorkflow).toBeDefined();
