...
-- ============================================================================
-- Table 1c: Users (Pipeline Engine identity, platform-agnostic)
-- ============================================================================
...
COMMENT ON TABLE remote_agent_users IS
  'Pipeline Engine-internal user identity. Created on first sight by any adapter; populated via per-platform user-info lookups.';
...
COMMENT ON TABLE remote_agent_user_identities IS
  'Maps platform-native user IDs (Slack U-ids, Telegram chat ids, GitHub logins, Discord snowflakes) to Pipeline Engine user UUIDs.';
...
-- One row per Pipeline Engine user; cascades on user deletion. github_user_id is the
...
-- and the column shape (text ids, camelCase columns) — Pipeline Engine never queries them
