...
| S5 | Engine behavior and deliberation store | Typed runtime files in `server/src/workflows/` not reserved by S4, S6, or S7; `server/src/config.ts`; `server/src/api/raindrop.ts`; `server/src/api/system.ts`; `server/src/agent/models.ts`; `.env.example`; `docs/{local-models-ollama,model-selection}.md`; `server/test/kady-node-executor.test.ts` (previously uninventoried), `server/test/raindrop-base-url.test.ts`, `server/test/supervisor-node-control-transport.test.ts`, `server/test/workflow-supervisor-{client,coordinator,protocol}.test.ts`; `server/src/agent/subagents.ts`; planned `server/src/{personality-store/**,agent/personality-store*.ts,api/personality-store*.ts}`; `server/vendor/pipeline-engine/packages/workflows/src/dag-executor.ts`; `server/vendor/pipeline-engine/packages/workflows/src/dag-executor.test.ts`; specifically the schema, state, registry, store, validation, execution, controller, service, evidence, Fusion, Lean4, import, and supervisor runtime/protocol files enumerated in `docs/inventory/ownership.json` |
