...
| S7 | Context/compaction watcher | `server/src/agent/{dag-fusion-bridge,session-registry}.ts`; `web/src/components/context-usage-indicator.{ts,tsx}`; planned `server/src/workflows/{context,compaction}-watcher*.ts`, `server/src/context/lateral-pass*`, and `web/src/components/{context,compaction}-watcher*.tsx` |
