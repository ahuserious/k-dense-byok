...
| --- | --- | --- |
| C1 | Vendored builder dist freshness and hosted build | `scripts/vendored-dist-*.mjs`; `package.json`; `.github/workflows/tests.yml`; `docs/preview-env.md`; handoff from S2 (2026-08-15): `start.mjs` vendored-dist build section; handoff from S11 (2026-08-15): `scripts/preview-up.mjs` `prepareVendoredDist` and prebuild-isolation ordering; handoff from S11 (2026-08-15): `scripts/preview-environment.mjs`, `scripts/preview-environment.test.mjs`, and `scripts/preview-launcher-observer.mjs`. |
...
Pull-request changed-path authorization derives the writer from the protected
branch naming contract `lane/<id>-<description>` (for example,
`lane/c1-dist-freshness` maps to writer `C1`) and compares against the exact PR
base SHA. Inventory uniqueness still runs independently on every workflow run;
push events have no PR base SHA and therefore emit an explicit authorization-skip
note.
