...
```bash
# Deterministic UI tier: 224 mocked items.
...
Resolved 2026-08-18 (N-10): the mocked tier's backend interception pattern used to be fixed to port `18000`, so running the mocked tier against a preview on any other backend port let every mocked request escape the fixture — on a host where something else answers on 18000, the specs then drove that instead and failed. `e2e/fixtures.ts` now derives the intercepted backend origins from `KADY_PORT` (default `18000`) plus `NEXT_PUBLIC_ADK_API_URL` when set, the same way the pipeline-engine origins were already derived. Pass `KADY_PORT` and `KADY_PIPELINE_ENGINE_PORT` matching the preview you booted and the mocked tier intercepts correctly on any ports; no `NEXT_PUBLIC_ADK_API_URL=…:18000` override is needed or wanted.
...
`playwright.cloud.config.ts` excludes `@live`: that topology deliberately exposes the frontend and vendored engine but not the real Kady backend. Its cloud-safe global setup fetches only the web root, so collection contains the 224 mocked items without pretending that a project page was warmed. The mocked-only tier can also be selected explicitly:
